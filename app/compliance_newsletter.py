...
def _ensure_storage(sheets: GoogleSheetsLive) -> None:
    sheets.ensure_sheet(SHEET_NAME, HEADERS)
    existing_headers = sheets.get_headers(SHEET_NAME)
...
def register_doi_pending(
    ...
    if existing_record.get("status") == "CONFIRMED":
        raise ValueError("E-Mail-Adresse ist bereits bestätigt.")

    subscriber_id = (
        existing_record.get("subscriber_id")
        or str(uuid.uuid4())
     )
    ...
    record = {
        ...
        "email": email_clean,
        "status": "PENDING",
        "doi_token_hash": _hash_value(token),
        "registered_at": now,
        "confirmed_at": "",
        "unsubscribed_at": "",
        "source": source,
    ...
    if existing_row_number is None:
        ...
        sheets.update_row(SHEET_NAME, existing_row_number, values)
...
def confirm_doi_token(token: str) -> bool:
    ...
    for row_number, record in enumerate(records, start=2):
        if (
            record.get("doi_token_hash") == token_hash
